...
while line:
    ...
    elif line.startswith("  Nbit-32K-1:"):
    ...
    elif line.startswith("  Nbit-16K-2:"):
        predictor_string = tokens[0].split(':')[0]
        correct_predictions = long(tokens[1])
        incorrect_predictions = long(tokens[2])
        x_Axis.append(predictor_string)
        mpki_Axis.append(incorrect_predictions / (total_ins / 1000.0))
    elif line.startswith("  Nbit-8K-4:"):
        predictor_string = tokens[0].split(':')[0]
        correct_predictions = long(tokens[1])
        incorrect_predictions = long(tokens[2])
        x_Axis.insert(0,predictor_string)
        mpki_Axis.append(incorrect_predictions / (total_ins / 1000.0))
    line = fp.readline()
...
plt.title("MPKI")
plt.savefig(raw_input("provide a filename:"),bbox_inches="tight")
